...
struct fd_uv_stream { union { uv_tcp_t tcp; uv_pipe_t pipe; } handle; int is_pipe; void* context; fd_uv_read_call read_callback; };
struct fd_uv_listener { union { uv_tcp_t tcp; uv_pipe_t pipe; } handle; int is_pipe; void* context; fd_uv_accept_call callback; };
...
static uv_stream_t* stream_handle(struct fd_uv_stream* stream) { return stream->is_pipe ? (uv_stream_t*) &stream->handle.pipe : (uv_stream_t*) &stream->handle.tcp; }
static uv_stream_t* listener_handle(struct fd_uv_listener* listener) { return listener->is_pipe ? (uv_stream_t*) &listener->handle.pipe : (uv_stream_t*) &listener->handle.tcp; }
static uv_handle_t* raw_stream_handle(struct fd_uv_stream* stream) { return (uv_handle_t*) stream_handle(stream); }

static void force_closed(uv_handle_t* handle) { free(handle); }
static void close_remaining(uv_handle_t* handle, void* unused) {
  (void) unused;
  if (!uv_is_closing(handle)) uv_close(handle, force_closed);
}
...
void fd_uv_loop_destroy(struct fd_uv_loop* loop) {
  ...
  uv_walk(&loop->loop, close_remaining, NULL);
...
static void on_accept(uv_stream_t* server, int status) {
  ...
  if (stream == NULL) { listener->callback(listener->context, NULL, status < 0 ? status : UV_ENOMEM); return; }
  stream->is_pipe = listener->is_pipe;
  if ((stream->is_pipe ? uv_pipe_init(server->loop, &stream->handle.pipe, 0) : uv_tcp_init(server->loop, &stream->handle.tcp)) != 0) { free(stream); listener->callback(listener->context, NULL, status < 0 ? status : UV_ENOMEM); return; }
  if (uv_accept(server, stream_handle(stream)) != 0) { uv_close(raw_stream_handle(stream), stream_closed); listener->callback(listener->context, NULL, status < 0 ? status : UV_ENOMEM); return; }
  stream_handle(stream)->data = stream;
...
struct fd_uv_listener* fd_uv_tcp_listen(struct fd_uv_loop* loop, void* context, fd_uv_accept_call callback, unsigned short* port) {
  ...
  if (listener == NULL || uv_tcp_init(&loop->loop, &listener->handle.tcp) != 0 || uv_ip4_addr("127.0.0.1", 0, &address) != 0 || uv_tcp_bind(&listener->handle.tcp, (const struct sockaddr*) &address, 0) != 0 || uv_listen(listener_handle(listener), 16, on_accept) != 0) { free(listener); return NULL; }
  listener->context = context; listener->callback = callback; listener_handle(listener)->data = listener;
  { int len = sizeof(address); if (uv_tcp_getsockname(&listener->handle.tcp, (struct sockaddr*) &address, &len) != 0) { uv_close((uv_handle_t*) &listener->handle.tcp, stream_closed); return NULL; } *port = ntohs(address.sin_port); }
  return listener;
}
struct fd_uv_listener* fd_uv_pipe_listen(struct fd_uv_loop* loop, void* context, fd_uv_accept_call callback, const char* name) {
  struct fd_uv_listener* listener = calloc(1, sizeof(*listener));
  if (listener == NULL) return NULL;
  listener->is_pipe = 1;
  if (uv_pipe_init(&loop->loop, &listener->handle.pipe, 0) != 0 || uv_pipe_bind(&listener->handle.pipe, name) != 0 || uv_listen(listener_handle(listener), 16, on_accept) != 0) { free(listener); return NULL; }
  listener->context = context; listener->callback = callback; listener_handle(listener)->data = listener;
  return listener;
}
void fd_uv_tcp_listener_close(struct fd_uv_listener* listener) { if (listener != NULL) uv_close((uv_handle_t*) listener_handle(listener), stream_closed); }
static void on_connect(uv_connect_t* request, int status) { struct fd_uv_connect* connect = request->data; if (status != 0) uv_close(raw_stream_handle(connect->stream), stream_closed); connect->callback(connect->context, status == 0 ? connect->stream : NULL, status); free(connect); }
void fd_uv_tcp_connect(struct fd_uv_loop* loop, void* context, fd_uv_connect_call callback, unsigned short port) {
  ...
  if (connect->stream == NULL || uv_tcp_init(&loop->loop, &connect->stream->handle.tcp) != 0 || uv_ip4_addr("127.0.0.1", port, &address) != 0 || uv_tcp_connect(&connect->request, &connect->stream->handle.tcp, (const struct sockaddr*) &address, on_connect) != 0) { free(connect->stream); free(connect); callback(context, NULL, UV_ECONNREFUSED); return; }
  stream_handle(connect->stream)->data = connect->stream; connect->request.data = connect;
}
void fd_uv_pipe_connect(struct fd_uv_loop* loop, void* context, fd_uv_connect_call callback, const char* name) {
  struct fd_uv_connect* connect = calloc(1, sizeof(*connect));
  if (connect == NULL) { callback(context, NULL, UV_ENOMEM); return; }
  connect->stream = calloc(1, sizeof(*connect->stream)); connect->context = context; connect->callback = callback;
  if (connect->stream == NULL) { free(connect); callback(context, NULL, UV_ENOMEM); return; }
  connect->stream->is_pipe = 1;
  if (uv_pipe_init(&loop->loop, &connect->stream->handle.pipe, 0) != 0) { free(connect->stream); free(connect); callback(context, NULL, UV_ECONNREFUSED); return; }
  stream_handle(connect->stream)->data = connect->stream; connect->request.data = connect;
  uv_pipe_connect(&connect->request, &connect->stream->handle.pipe, name, on_connect);
}
void fd_uv_tcp_stream_set_context(struct fd_uv_stream* stream, void* context) { stream->context = context; }
int fd_uv_tcp_stream_read(struct fd_uv_stream* stream, fd_uv_read_call callback) { stream->read_callback = callback; return uv_read_start(stream_handle(stream), alloc_read, on_read); }
void fd_uv_tcp_stream_close(struct fd_uv_stream* stream) { if (stream != NULL) uv_close(raw_stream_handle(stream), stream_closed); }
